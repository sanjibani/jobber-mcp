...
The audit sink defaults to stderr. Override via env var:
    ...
    JOBBER_MCP_AUDIT_LOG=/var/log/hawksoft-mcp/audit.jsonl
...
def _resolve_sink() -> Any:
    ...
    """
    path = os.environ.get("JOBBER_MCP_AUDIT_LOG")
    if path:
        try:
...
def audit_tool_call(tool: str, args: dict[str, Any]) -> Iterator[_AuditRecord]:
    ...
    Writes to stderr by default; set JOBBER_MCP_AUDIT_LOG to a file path
    to redirect.
    """
