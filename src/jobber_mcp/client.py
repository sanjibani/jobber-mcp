"""Async HTTP client for Jobber MCP."""
...
class JobberError(RuntimeError):
    """Base exception for Jobber MCP client errors."""
...
class JobberClient:
    """Async client for Jobber MCP.

    Authentication: HTTP Basic. Pass credentials explicitly OR set
    ``JOBBER_USERNAME`` and ``JOBBER_PASSWORD`` in the environment.
    """
    ...
    def __init__(
        ...
        *,
        ...
        username = username or os.environ.get("JOBBER_USERNAME")
        password = password or os.environ.get("JOBBER_PASSWORD")
        if not username or not password:
            raise JobberAuthError(
                "Jobber MCP credentials missing. Set JOBBER_USERNAME and "
                "JOBBER_PASSWORD environment variables, or pass them to the client."
            )
    ...
    async def _request(
        ...
        *,
        ...
        if response.status_code == 401:
            raise JobberAuthError(
                "Jobber MCP rejected the credentials (HTTP 401).", 401
            )
        if response.status_code == 403:
            raise JobberAuthError(
                "Jobber MCP denied access (HTTP 403).", 403
            )
        if not 200 <= response.status_code < 300:
            ...
            raise JobberAPIError(
                f"Jobber MCP returned HTTP {response.status_code}",
