...
from jobber_mcp.audit import _sanitize_args, audit_tool_call
...
def test_audit_tool_call_writes_to_file_when_env_set(tmp_path) -> None:
    """JOBBER_MCP_AUDIT_LOG overrides default sink to a file path."""
    log_file = tmp_path / "audit.jsonl"
    os.environ["JOBBER_MCP_AUDIT_LOG"] = str(log_file)
    try:
        with audit_tool_call("file_tool", {"k": "v"}) as _:
            _.set_result("ok")
    finally:
        os.environ.pop("JOBBER_MCP_AUDIT_LOG", None)
...
def test_audit_tool_call_never_breaks_tool_on_sink_failure() -> None:
    ...
    os.environ["JOBBER_MCP_AUDIT_LOG"] = "/nonexistent/dir/audit.jsonl"
    try:
